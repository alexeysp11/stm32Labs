...
**Асинхронная передача данных** (англ. **Asynchronous Transmission**) - это режим последовательной передачи, в котором данные передаются в виде непрерывного потока байтов, разделенных стартовыми и стоповыми битами.
...
Например, для *UART* пакет выглядит следующим образом: 
